...
auto Board::bitboardsTensor() const -> std::vector<float> {
  std::vector<float> tensor(1 * 12 * 8 * 8, 0.0f);

  for (auto r = 0; r < 8; r++) {
    for (auto c = 0; c < 8; c++) {
      auto square = Utils::Square::GetSquareIndex(c, r);
      auto piece = pieceSet[square];

      if (piece.Type == PieceType::None) {
        continue;
      }

      auto pos = (64 * 6) * piece.Color + 64 * piece.Type + Utils::Square::MirrorSquare(square);
      tensor[pos] = 1.0;
    }
  }
  return tensor;
