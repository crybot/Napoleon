...
namespace Napoleon
{
    const int Search::AspirationValue = 20;
    ...
    int Search::search(int depth, int alpha, int beta, int ply, Board& board)
    {
        ...
        {
            if (board.IsCheck())
                alpha = -Constants::Infinity + ply; // return best score for the deepest mate
            else
    ...
    int Search::quiescence(int alpha, int beta, Board& board)
    {
        ...
        for(int i=0; i<moves.count; i++)
        {
            ...
            if(!inCheck)
            {
                if (Constants::Piece::PieceValue[board.PieceOnSquare(move.ToSquare()).Type] + stand_pat + 200 < alpha && !move.IsPromotion())
                    continue;
            }
    ...
    std::string Search::GetInfo(Board& board, Move toMake, int score, int depth, int lastTime)
    {
        ...
        info << "depth " << depth << " score cp " << score << " time " << searchInfo.ElapsedTime() << " nodes "
